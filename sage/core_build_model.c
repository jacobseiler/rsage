...
void evolve_galaxies(int halonr, int ngal, int tree)	// Note: halonr is here the FOF-background subhalo (i.e. main halo) 
{
  ...
  for(step = 0; step < STEPS; step++)
  {
    ...
    if (Gal[p].EjectedMass > 0.0)
      XASSERT(Gal[p].EjectedMass / (Gal[p].EjectedMassSN + Gal[p].EjectedMassQSO) > 0.99, "EjectedMass %.4e\tSN %.4e\tQSO %.4f\n", Gal[p].EjectedMass, Gal[p].EjectedMassSN, Gal[p].EjectedMassQSO);
