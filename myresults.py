...
def plot_quasars_count(SnapList, PlotList, N_quasars_z, N_quasars_boost_z, N_gal, mean_quasar_activity, std_quasar_activity, N_halo, fesc_prescription, simulation_norm, FirstFile, LastFile, NumFile, model_tags, output_tag):
    ...
    for model_number in range(0, len(SnapList)): # Does this for each of the models. 
        ...
        if rank == 0: 
            ...
            ax1.plot(np.nan, np.nan, color = 'r', linestyle = PlotScripts.linestyles[0], label = "Quasar Density")
            ax1.plot(np.nan, np.nan, color = 'r', linestyle = PlotScripts.linestyles[1], label = "Galaxy Density")
        ...
        for fnr in range(FirstFile[model_number] + rank, LastFile[model_number]+1, size): # Divide up the input files across the processors.            
            ...
            while(keep_files == 1): 
                ...
                for snapshot_idx in range(0, len(SnapList[current_model_number])): # Now let's calculate stats for each required redshift.                
                    ...
                    print(sum(photons_HI_gal_nonlog[0:10]))
