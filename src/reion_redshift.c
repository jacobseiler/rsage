                              ...
                              char *PhotionDir, char *FileNameGalaxies, char *ReionredshiftName) 
{
  ...
  char reion_fname[MAX_STRING_LEN];
  ...
  {
    printf("Generating new reionization redshift grid.\n");
  ...
  {
    printf("Reading in old reionization redshift grid.\n");
  ...
  printf("Reading in the XHII grid for Snapshot %d.\n", SnapNum);
  double *XHII_grid;
  ...
  snprintf(XHII_fname, MAX_STRING_LEN - 1, "%s/%s_XHII_%03d", PhotionDir, FileNameGalaxies, SnapNum);
  ...
  num_read = fread(XHII_grid, sizeof(*(XHII_grid)), CUBE(GridSize), XHII_file);  
  if (num_read != CUBE(GridSize))
  {
    fprintf(stderr, "Attempted to read %d elements but only read %d elements from file %s\n", 
