...
def Plot_Histogram(Data, Labels_Graph, Limits, Bin_Width, Normalization, Weights, Log, Labels_Axis, Legend_Location, Output_Tag, Output_Format):
...
# A file in the form '*Output_Tag**Output_Format' saved in the current working directory.
    ...
    for i in xrange(0, len(Data)):
        ...
        plt.plot(Bin_Middle, counts / Normalization, colours[i], label = Labels_Graph[i])
...
def Plot_Scatter_TwoHists(Data_x, Data_y, Labels_Graph, Limits, Hist_Limits, Bin_Width, Normalization, Weights, Labels_Axis, Output_Tag, Output_Format):
...
# A file in the form '*Output_Tag**Output_Format' saved in the current working directory.
    ...
    leg = ax3.legend(bbox_to_anchor = (1.6, 2), scatterpoints=1, labelspacing=0.1, handletextpad = 0.05)
    leg.draw_frame(False)  # Don't want a box frame
    for t in leg.get_texts():  # Reduce the size of the text
        t.set_fontsize('medium')
...
def Plot_Scatter(Data_x, Data_y, Labels_Graph, Limits, Log, Labels_Axis, Legend_Location, Output_Tag, Output_Format):
    ...
    for i in xrange(0, len(Data_x)):
        ...
        if (len(Data_x[i]) > dilute):
            w = sample(range(len(Data_x[i])), dilute)
            ax.scatter(Data_x[i][w], Data_y[i][w], color = colours[i], marker = markers[i], label = Labels_Graph[i], alpha = 0.8)
        else:
            w = np.arange(0,len(Data_x[i]))
            ax.scatter(Data_x[i], Data_y[i], color = colours[i], marker = markers[i], label = Labels_Graph[i], alpha = 0.8)
