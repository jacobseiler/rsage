...
def my_load_data(fname):
    ...
    fesc = data[:,1]
    Mvir = data[:,2]
    Ngamma = data[:,3] # Units of 1.0e50 Photons/s.
    Ngammafesc = data[:,4] # Units of 1.0e50 Photons/s.
...
def plot_fesc_z(filename_base, PlotSnapshot, model_tags, output_tag):
    ...
    ax1.axhline(0.05, 0, 100, color ='k', linewidth = PlotScripts.global_linewidth, linestyle = '-.')
    #ax1.axvline(resolution, color = 'k', linewidth = PlotScripts.global_linewidth, linestyle = '-.')   
    ax1.text(11.0, 0.12, r"$f_\mathrm{esc} = 0.10$", color = 'k', size = PlotScripts.global_fontsize - 2)
    ax1.text(9.25, 0.01, r"$f_\mathrm{esc, \: base}$", color = 'k', size = PlotScripts.global_fontsize - 2)
    ...
    for model_number in range(0, len(base_grid_name)):
        ...
        for count, snapshot_idx in enumerate(SnapList[model_number]):  
            ...
            if (count != 0):
                ...
                print(AllVars.SnapZ[SnapList[model_number][count]])
                print(dt_nion[model_number][count-1])
...
if __name__ == '__main__':
    ...
    PlotSnapshot = [[29, 40, 49, 76, 93], [29, 40, 49, 76, 93]]
    #PlotSnapshot = [[27, 40, 49, 76, 93]]

    #model_tags = [r"$f_\mathrm{esc} \: \propto \: \mathrm{Quasar, 0.05,1.00,2.00}$", r"$f_\mathrm{esc} = 0.10$"]
    model_tags = [r"$f_\mathrm{esc} \: \propto \: \mathrm{Quasar \: Activity}$", r"$f_\mathrm{esc} = 0.10$"]
